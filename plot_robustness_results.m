...
% Updated: Simplified label placement (removed collision avoidance logic).
        ...
        if isfield(divergence_data, div_field)
            ...
            if actually_diverged
                ...
                if ~isempty(idx)
                    ...
                    % --- Simple Label (EKF) ---
                    text(ax3, div_point, final_y, sprintf('EKF-%s: %.1fx', state_names{state_idx}, div_point), ...
                         'Color', colors(state_idx,:), 'FontSize', 8, 'FontWeight', 'bold', ...
            ...
            if actually_diverged_run
                ...
                if ~isempty(idx)
                    h_run = plot(ax3, x_all(idx), y_all_run_pos(idx), '--', ...
                             ...
                             'DisplayName', sprintf('%s (Run)', state_names{state_idx}));
                    ...
                    % --- Simple Label (Running) ---
                    text(ax3, div_point_run, final_y_run, sprintf('Run-%s: %.1fx', state_names{state_idx}, div_point_run), ...
                         'Color', colors(state_idx,:)*0.8, 'FontSize', 8, ...
        ...
        if isfield(divergence_data, div_field)
            ...
            if actually_diverged
                ...
                if ~isempty(idx)
                    ...
                    % --- Simple Label (EKF) ---
                    text(ax4, div_point, final_y, sprintf('%s: %.1fx', state_names{state_idx}, div_point), ...
                         'Color', colors(state_idx,:), 'FontSize', 8, 'FontWeight', 'bold', ...
                         'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom', ...
                         'BackgroundColor', [1 1 1 0.7]);
                end
        ...
        if isfield(divergence_data, 'rmse_values_running') && state_idx ~= 3 && state_idx ~= 4
            ...
            if actually_diverged_run
                ...
                if ~isempty(idx)
                    ...
                    % --- Simple Label (Run) ---
                    text(ax4, div_point_run, final_y_run, sprintf('%s (Run)', state_names{state_idx}), ...
                         'Color', colors(state_idx,:)*0.8, 'FontSize', 8, ...
                         'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', ...
                         'BackgroundColor', [1 1 1 0.7]);
                end
...
if isfield(divergence_data, 'state_names')
    ...
    summary{end+1} = 'State    EKF Status     EKF End(x)   |   Run Status     Run End(x)';
    summary{end+1} = '------   -----------    ----------   |   -----------    ----------';
    for state_idx = 1:6
        data_state_name = divergence_data.state_names{state_idx};
...
sgtitle({'Extended Kalman Filter vs Running Mean: Robustness Analysis', 'Split view: x/dx (log) vs. y/dy/θ/dθ (linear)'}, 'FontSize', 14, 'FontWeight', 'bold');
